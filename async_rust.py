...
class MyCLibrary:
    def __init__(self):
        ...
        self.lib.MyCLibrary_Create.restype = ctypes.c_void_p
        
        self.lib.MyCLibrary_Destroy.argtypes = [ctypes.POINTER(ctypes.c_void_p)]
        
        self.lib.MyCLibrary_SleepAndAdd.argtypes = [
        ...
        self.lib.MyCLibrary_GetCompletedRequests.argtypes = [
            ctypes.c_void_p,  # my_library
            ctypes.POINTER(CCompletedRequest),  # completed_requests
            ctypes.c_uint64,  # completed_requests_len
            ctypes.c_uint64,  # wait_num
        ]
        self.lib.MyCLibrary_GetCompletedRequests.restype = ctypes.c_uint64
        
        # Create the library instance
        self.my_library = self.lib.MyCLibrary_Create()
    ...
    def __del__(self):
        ...
        if hasattr(self, 'my_library'):
            self.lib.MyCLibrary_Destroy(ctypes.byref(ctypes.c_void_p(self.my_library)))
    ...
    async def sleep_and_add(self,  left: int, right: int) -> int:
        ...
        self.lib.MyCLibrary_SleepAndAdd(self.my_library, userdata, left, right, ctypes.byref(result))
    ...
    async def check_completed_requests(self):
        ...
        while True:
            count = self.lib.MyCLibrary_GetCompletedRequests(
                self.my_library,
                completed_requests,
